...
| 2026-04-29 | **Schema / Migração multi-tenant:** detalhada em [13-EMPRESA-IAM-MULTI-TENANT-E-DEPLOY.md](13-EMPRESA-IAM-MULTI-TENANT-E-DEPLOY.md) a proposta de `schema.prisma` alvo e o plano de migração em fases do modelo atual (`Usuario` por `UnidadeFabril` + `perfil`) para `Empresa`, `UsuarioEmpresa`, `Cargo`, `Permissao`, `UsuarioCargo` e `ConviteAcesso`. |
| 2026-04-29 | **Prisma aditivo:** `backend/prisma/schema.prisma` expandido com `Empresa`, estados de vínculo, RBAC (`Cargo`, `Permissao`, `CargoPermissao`, `UsuarioCargo`), `ConviteAcesso` e `empresa_id` opcional nas tabelas operacionais centrais para permitir backfill sem remoção imediata do legado. |
