...
import { setupTopology, publishEvent } from './rabbitmq.shared';
...
export class EventPublisherService implements OnModuleDestroy {
  ...
  async publishWebhookDeliver(
    event: Omit<WebhookDeliverEvent, 'version'>,
  ): Promise<boolean> {
    const channel = await this.ensureChannel();
    if (!channel) return false;

    await publishEvent(channel, MESSAGING.routingKeys.webhookDeliver, {
      version: 1,
      ...event,
    });
    return true;
  ...
  async publishEmailSend(
    event: Omit<EmailSendEvent, 'version'>,
  ): Promise<boolean> {
    const channel = await this.ensureChannel();
    if (!channel) return false;

    await publishEvent(channel, MESSAGING.routingKeys.emailSend, {
      version: 1,
      ...event,
    });
    return true;
