...
import type { OrdemServicoComentarioItem } from '../../domain/entities/ordem-servico';
import {
...
@Injectable()
export class ListOrdemServicoComentariosUseCase {
  constructor(
  ...
  async execute(
    ...
    await this.assertOrdemExists(idUnidade, idOrdemServico);
    return this.ordens.listComentarios(idOrdemServico);
  }

  private async assertOrdemExists(idUnidade: string, idOrdemServico: string) {
